...
def train_one_epoch(model, dataloader, optimizer, device, beta, fast_test=False):
    ...
    if n > 0:
        return total_train_loss / n, total_recon_loss / n, total_kld_loss / n, total_grad_norm / n, (gate_weights_accum / n if isinstance(gate_weights_accum, torch.Tensor) else None)
    return 0, 0, 0, 0, None
...
def main(args):
    ...
    with open(csv_path, mode='w', newline='') as f:
        ...
        csv_writer.writerow(['Epoch', 'Beta', 'Time_s', 'Train_Total_Loss', 'Train_Recon_MSE', 'Train_KLD_Loss', 'Val_Recon_MSE', 'Grad_Norm', 'Gate_Exp_1', 'Gate_Exp_2', 'Gate_Exp_3'])
    ...
    for epoch in range(args.epochs):
        ...
        t_loss, t_recon, t_kld, t_grad, t_gates = train_one_epoch(model, train_loader, optimizer, device, beta, fast_test=args.fast_test)
        ...
        print(f"Epoch [{epoch+1}/{args.epochs}] | Time: {epoch_time:.1f}s | Beta: {beta:.3f} | " +
              f"Train Loss: {t_loss:.4f} (MSE: {t_recon:.4f}, KLD: {t_kld:.4f}) | " +
              f"Val Recon: {v_recon:.4f} | Grad: {t_grad:.2f}")
        ...
        with open(csv_path, mode='a', newline='') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow([epoch+1, f"{beta:.5f}", f"{epoch_time:.2f}", f"{t_loss:.6f}", f"{t_recon:.6f}", f"{t_kld:.6f}", f"{v_recon:.6f}", f"{t_grad:.4f}", f"{g1:.4f}", f"{g2:.4f}", f"{g3:.4f}"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='./proposed/TGCN_cVAE_MoE/weights', help='Directory to save model weights')
